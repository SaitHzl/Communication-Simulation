...
public class Operator {
	...
	double calculateMessageCost(int quantity, Customer customer, Customer other) {
		if(customer.getOperator().ID == other.getOperator().ID) {
			return (quantity*customer.getOperator().messageCost) - ((quantity*customer.getOperator().messageCost) - ((quantity*customer.getOperator().discountRate)/100));
		}
		else {
